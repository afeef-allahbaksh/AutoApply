...
- Local FastAPI dashboard (`src/ui/`) — kanban view of applications with drag-and-drop, jobs browser with track-to-application, companies add, profile/responses settings, IMAP inbox setup. HTMX + Jinja2 + Sortable.js. Dark-theme design tokens (Geist Sans + Mono, deep canvas `#0d0e13`, indigo accent `#818cf8`, three-tier surface elevation, smooth motion) in `base.html`
- IMAP inbox integration (`src/inbox/`) — read-only `imaplib` scrape running on a background daemon thread per profile, streaming proposals to disk as classification chunks complete. Two-phase bulk FETCH (headers then bodies), prefilter heuristic, Claude (Haiku 4.5) OR regex-only "free mode" classifier, multi-signal matcher (thread-id → company → sender domain → subject prefix → body scan), dedup against existing kanban entries. Auth via app password instead of OAuth so users don't need their own Google Cloud project. Cancel + interrupted-state detection + atomic status writes. Never auto-applies silently — every change goes through the review queue
...
13. ~~Gmail inbox scrape with proposal review queue (auto-add new apps, auto-update on follow-ups)~~ (done — switched OAuth → IMAP for distribution-friendliness)
14. ~~Streaming background sync pipeline with progress polling, cancel button, atomic status writes~~ (done)
15. ~~Free mode regex classifier — zero-API alternative for users out of credits~~ (done)
16. ~~Matcher hardening with multi-signal company candidate extraction~~ (done)
17. ~~Dark theme UI with motion polish~~ (done)
18. Ashby support (v2)
19. Cold email composer + tracker (v2 — placeholder reserved in dashboard sidebar)
20. Workday support (later, if at all)
