...
- **Inbox integration** — `src/inbox/` is a read-only IMAP scraper. Pipeline: `fetch.list_messages_since(creds, since_dt)` (stdlib `imaplib`, IMAPS, `SINCE` search on INBOX) → `sync._prefilter` (drops LinkedIn/Indeed/etc.) → `classify.classify_messages` (batched Claude call, returns `new_application` / `status_update` / `ignore` plus company, role, status, confidence) → `matcher.match_application` (thread-id wins; fuzzy company; narrow by role tokens). Thread ids approximate Gmail's by walking RFC 822 `References` / `In-Reply-To` headers — first message uses its own Message-ID; replies share the root via References[0]. Results land as proposals in `profiles/{name}/imap/proposals.json`; `state.json` tracks `last_sync_at` and `processed_message_ids` (capped at 5000) so re-syncs skip already-seen messages. Proposals are reviewed manually — the user clicks Apply/Dismiss in the Applications-page review queue. **Never auto-apply silently.** Apply mutates `applications.json` via `_save_applications` (so schema validation runs), tagging entries with `source="email"` and appending the thread id to `email_thread_ids`. Credentials live in `profiles/{name}/imap/credentials.json` as `{email, password, server, port}` — Gmail-default but any IMAPS host works. App password is full-mailbox-access (not just read like an OAuth refresh token would be); the trade is ~2 minutes of user setup vs. each user registering their own Google Cloud OAuth client
